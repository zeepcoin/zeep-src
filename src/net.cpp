...
static const char *strDNSSeed[][2] = {
    {"209.182.216.96", "209.182.216.96"},
	{"209.182.216.95", "209.182.216.95"}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Zeepcoin is probably already running."), addrBind.ToString().c_str());
        else
