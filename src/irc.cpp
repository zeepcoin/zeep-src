...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ZeepcoinTEST2\r");
            Send(hSocket, "WHO #ZeepcoinTEST2\r");
        } else {
            // randomly join #Zeepcoin00-#Zeepcoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #Zeepcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Zeepcoin%02d\r", channel_number).c_str());
        }
