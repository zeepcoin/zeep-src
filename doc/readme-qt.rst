Zeepcoin-qt: Qt4 GUI for Zeepcoin
===============================
...
Alternatively, install Qt Creator and open the `Zeepcoin-qt.pro` file.

An executable named `Zeepcoin-qt` will be built.
...
A warning for people using the *static binary* version of Zeepcoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Zeepcoin is linked against libdb5.3.
...
installed causes Zeepcoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
